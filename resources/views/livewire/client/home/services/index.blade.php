<div>
<section
    class="container gap-3 mt-3 overflow-x-auto services d-flex px-lg-0 flex-md-wrap align-items-center justify-content-center gap-lg-5 mt-lg-5">
    <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
        <a href="#">
            <img width="50" src="/client/assets/services/jet.png" alt="digikala jet" />
        </a>
        <p class="fs-8">دیجی‌کالا جت</p>
    </div>
    <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
        <a href="#">
            <img width="50" src="/client/assets/services/digipay.png" alt="digipay" />
        </a>
        <p class="fs-8">دیجی‌پی</p>
    </div>
    <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
        <a href="#">
            <img width="50" src="/client/assets/services/style.png" alt="Ds" />
        </a>
        <p class="fs-8">%۷۰ تخفیف‌ پلاس</p>
    </div>
    <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
        <a href="#">
            <img width="50" src="/client/assets/services/omde.png" alt="bussiness" />
        </a>
        <p class="fs-8">موبایل کارکرده</p>
    </div>
    <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
        <a href="#">
            <img width="50" src="/client/assets/services/ersal.png" alt="send" />
        </a>
        <p class="fs-8">گیفت کارت</p>
    </div>
    <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
        <a href="#">
            <img width="50" src="/client/assets/services/lavazem.png" alt="pencile" />
        </a>
        <p class="fs-8">بازی برنده‌ها</p>
    </div>
    <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
        <a href="#">
            <img width="50" src="/client/assets/services/vaght.png" alt="time to buy" />
        </a>
        <p class="fs-8">سوپرمارکت پرتخفیف</p>
    </div>

    <!-- More -->
    <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
        <a href="#" class="p-3 bg-secondary-subtle text-black-50 rounded-5" data-bs-toggle="modal"
            data-bs-target="#exampleModal">
            <svg width="23" height="23" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                class="bi bi-three-dots" viewBox="0 0 16 16">
                <path
                    d="M3 9.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3m5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3m5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3" />
            </svg>
        </a>
        <p class="fs-8">بیشتر</p>
    </div>

    <!-- Modal -->
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content rounded-4">
                <div class="p-4 modal-header">
                    <h6 class="modal-title fw-medium" id="exampleModalLabel">خدمات دیجی‌کالا</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="m-5 modal-body">
                    <!-- First section -->
                    <div class="flex-wrap gap-5 mb-5 d-flex align-items-center justify-content-center">
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/jet.png" alt="digikala jet" />
                            </a>
                            <p class="fs-8">دیجی‌کالا جت</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/digipay.png" alt="digipay" />
                            </a>
                            <p class="fs-8">دیجی‌پی</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/style.png" alt="Ds" />
                            </a>
                            <p class="fs-8">%۷۰ تخفیف‌ پلاس</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/omde.png" alt="bussiness" />
                            </a>
                            <p class="fs-8">موبایل کارکرده</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/ersal.png" alt="send" />
                            </a>
                            <p class="fs-8">گیفت کارت</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/lavazem.png" alt="pencile" />
                            </a>
                            <p class="fs-8">بازی برنده‌ها</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/vaght.png" alt="time to buy" />
                            </a>
                            <p class="fs-8">سوپرمارکت پرتخفیف</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/jet.png" alt="digikala jet" />
                            </a>
                            <p class="fs-8">دیجی‌کالا جت</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/digipay.png" alt="digipay" />
                            </a>
                            <p class="fs-8">دیجی‌پی</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/style.png" alt="Ds" />
                            </a>
                            <p class="fs-8">%۷۰ تخفیف‌ پلاس</p>
                        </div>
                        <div class="gap-2 d-flex flex-column align-items-center justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/omde.png" alt="bussiness" />
                            </a>
                            <p class="fs-8">موبایل کارکرده</p>
                        </div>
                    </div>

                    <!-- Digi Plus -->
                    <div class="mt-5">
                        <div class="gap-2 d-flex flex-column justify-content-center">
                            <a href="#">
                                <img width="50" src="/client/assets/services/digiplus.png" alt="digiplus" />
                            </a>
                            <p class="fs-8">دیجی‌پلاس</p>
                        </div>
                    </div>

                    <!-- other digikala services -->
                </div>
            </div>
        </div>
    </div>
</section>
</div>
